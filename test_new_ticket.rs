...
fn main() {
    ...
    let ticket_content = std::fs::read_to_string("current_ticket.txt").expect("Failed to read ticket file");
    let ticket = ticket_content.trim().strip_prefix("ticket:").unwrap_or(&ticket_content);

    println!("🎫 Testing full ticket from file...");
    ...
    match parse_ticket(ticket) {
        Ok(endpoint_addr) => {
            ...
            println!("✅ Connection should now work!");
        }
        Err(e) => {
