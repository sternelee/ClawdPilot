...
import { FiSend, FiPlus, FiX, FiFolder, FiStopCircle } from "solid-icons/fi";
...
export const ChatInput: Component<ChatInputProps> = (props) => {
	...
	const handleKeyDown = (e: KeyboardEvent) => {
		if (showSlashSuggestions()) {
			...
			if (e.key === "Tab" || e.key === "Enter") {
				...
				if (item) props.onSelectSlash?.(item.value || item.name);
				return;
			}
			if (e.key === "Escape") {
				e.preventDefault();
		...
		if (showMentionSuggestions()) {
			...
			if (e.key === "Tab" || e.key === "Enter") {
				e.preventDefault();
				const item = mentionSuggestions()[activeSlashIndex()];
				if (item) props.onSelectMention?.(item.path);
				return;
			}
			if (e.key === "Escape") {
				e.preventDefault();
	...
	return (
		<div
			class={cn(
				"relative bg-base-100 border-t border-base-content/10",
				props.class,
			)}
		>
			{/* Slash Suggestions */}
					...
					<For each={mentionSuggestions()}>
						{(item, index) => (
							...
							>
								...
								<span class="text-sm font-mono text-base-content truncate">
									{item.path}
								</span>
							</button>
						)}
			...
			<div class="flex flex-col px-3 py-2.5 gap-2">
				...
				{/* Textarea Row */}
				<div
					class="flex items-end gap-1.5 rounded-[var(--as-radius-lg,1rem)] border transition-colors duration-150"
					classList={{
