		...
		setState(
			produce((s: ChatState) => {
				...
				if (perm) {
					...
					perm.status =
						response === "denied" || response === "abort"
							? "denied"
							: "approved";
				}
			}),
	...
	const clearPermission = (sessionId: string, permissionId: string) => {
		setState(
			produce((s: ChatState) => {
				const permissions = s.pendingPermissions[sessionId];
				if (!permissions) return;
				s.pendingPermissions[sessionId] = permissions.filter(
					(p) => p.id !== permissionId,
				);
			}),
		);
	...
	const clearQuestion = (sessionId: string, questionId: string) => {
		setState(
			produce((s: ChatState) => {
				const questions = s.pendingQuestions[sessionId];
				if (!questions) return;
				s.pendingQuestions[sessionId] = questions.filter(
					(q) => q.id !== questionId,
				);
			}),
		);
	...
	const removeAttachment = (sessionId: string, attachmentId: string) => {
		setState(
			produce((s: ChatState) => {
				...
				s.attachments[sessionId] = attachments.filter(
					(a) => a.id !== attachmentId,
				);
			}),
		);
