...
const STORAGE_KEY = 'riterm-toolbar-preferences';
...
export function useToolbarPreferences() {
  const [preferences, setPreferences] = createSignal<ToolbarPreferences>(DEFAULT_PREFERENCES);
  ...
  onMount(() => {
    try {
      ...
      console.error('[useToolbarPreferences] Failed to load preferences:', error);
    } finally {
      setIsLoaded(true);
    }
  });
  ...
  createEffect(() => {
    ...
    try {
      ...
      console.error('[useToolbarPreferences] Failed to save preferences:', error);
    }
  });
  ...
  const importPreferences = (json: string): boolean => {
    try {
      ...
      console.error('[useToolbarPreferences] Failed to import preferences:', error);
      return false;
    }
  };
